...
const ShipmentPageContent = ({ id }: { id: string }) => {
  const { data: shipmentData, isLoading, isError } = useShipment(id as string);

  if (isLoading)
    return (
      <div className="min-h-[calc(100vh-100px)] md:min-h-[calc(100vh-125px)]  flex flex-col justify-center items-center padding-x rounded-lg">
        <Loader size={48} />
      </div>
    );
  ...
  if (isError)
    return (
      <div className="mt-16.5 padding-x rounded-lg">
        <h2 className="mt-6 md:mt-10 text-2xl md:text-[32px] font-bold leading-10 text-red-400">
          Shipment not found
        </h2>
      </div>
    );
  ...
  return (
    <div className="mt-16.5 padding-x rounded-lg">
      ...
      <div className="mt-6 md:mt-10 p-4 md:p-8 bg-white rounded-lg">
        ...
        <h2 className="text-lg font-semibold leading-7 font-roboto">Summary</h2>
        ...
        <div
          className={`${true ? "justify-between" : "justify-end"} flex gap-2`}
        >
          {true && (
            <Button
              variant="outline"
          ...
          <Button className="mt-10 submit-button w-fit! px-[87px]">Done</Button>
        </div>
      </div>
    </div>
  );
